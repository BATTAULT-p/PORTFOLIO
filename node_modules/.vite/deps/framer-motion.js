  ...
  function MotionComponent(props, externalRef) {
    ...
    if (!isStatic && isBrowser) {
      ...
      if (context.visualElement) {
        features = context.visualElement.loadFeatures(configAndProps, lazyStrictMode, preloadedFeatures, projectionId, projectionNodeConstructor || featureDefinitions.projectionNodeConstructor, initialLayoutGroupConfig);
      }
    }
    return React.createElement(VisualElementHandler, { visualElement: context.visualElement, props: configAndProps }, features, React.createElement(MotionContext.Provider, { value: context }, useRender(Component2, props, projectionId, useMotionRef(visualState, context.visualElement, externalRef), visualState, isStatic, context.visualElement)));
  }
...
function keyframes({ keyframes: keyframeValues, ease = easeInOut, times, duration = 300 }) {
  ...
  const absoluteTimes = convertOffsetToTimes(times && times.length === keyframes.length ? times : defaultOffset(keyframeValues), duration);
...
function spring(_a) {
  var _b = _a, { keyframes: keyframes2, restSpeed = 2, restDelta = 0.01 } = _b, options = __objRest(_b, ["keyframes", "restSpeed", "restDelta"]);
...
function animate(_a) {
  var _b = _a, { duration, driver = framesync, elapsed = 0, repeat: repeatMax = 0, repeatType = "loop", repeatDelay = 0, keyframes: keyframes2, autoplay = true, onPlay, onStop, onComplete, onRepeat, onUpdate, type = "keyframes" } = _b, options = __objRest(_b, ["duration", "driver", "elapsed", "repeat", "repeatType", "repeatDelay", "keyframes", "autoplay", "onPlay", "onStop", "onComplete", "onRepeat", "onUpdate", "type"]);
  var _a2, _b2;
  let driverControls;
  let repeatCount = 0;
  ...
  const animation = animator(__spreadProps(__spreadValues({}, options), {
    duration,
...
function inertia({ keyframes: keyframes2, velocity = 0, min, max, power = 0.8, timeConstant = 750, bounceStiffness = 500, bounceDamping = 10, restDelta = 1, modifyTarget, driver, onUpdate, onComplete, onStop }) {
  ...
  function startAnimation(options) {
    ...
    currentAnimation = animate(__spreadProps(__spreadValues({
      keyframes: [0, 1],
      velocity: 0
    }, options), {
      driver,
...
function animateTarget(visualElement, definition, { delay: delay2 = 0, transitionOverride, type } = {}) {
  ...
  for (const key in target) {
    ...
    let valueTransition = __spreadValues({ delay: delay2, elapsed: 0 }, transition);
  ...
  loadFeatures(renderedProps, isStrict, preloadedFeatures, projectionId, ProjectionNodeConstructor, initialLayoutGroupConfig) {
    ...
    for (let i = 0; i < numFeatures; i++) {
      ...
      if (isEnabled(renderedProps) && Component2) {
        features.push((0, import_react26.createElement)(Component2, __spreadProps(__spreadValues({
          key: name
        }, renderedProps), {
          visualElement: this
        })));
...
function mixValues(target, follow, lead, progress3, shouldCrossfadeOpacity, isOnlyMember) {
  if (shouldCrossfadeOpacity) {
    target.opacity = mix(0, lead.opacity !== void 0 ? lead.opacity : 1, easeCrossfadeIn(progress3));
...
var LayoutGroup = ({ children, id: id4, inheritId, inherit = true }) => {
  ...
  const memoizedContext = (0, import_react34.useMemo)(() => __spreadProps(__spreadValues({}, context.current), { forceRender }), [key]);
...
function MotionConfig(_a) {
  ...
  config = __spreadValues(__spreadValues({}, (0, import_react35.useContext)(MotionConfigContext)), config);
...
function ReorderGroup(_a, externalRef) {
  ...
  return React9.createElement(Component2, __spreadProps(__spreadValues({}, props), { ref: externalRef }), React9.createElement(ReorderContext.Provider, { value: context }, children));
}
...
function ReorderItem(_a, externalRef) {
  ...
  return React10.createElement(Component2, __spreadProps(__spreadValues({ drag: axis }, props), { dragSnapToOrigin: true, style: __spreadProps(__spreadValues({}, style2), { x: point2.x, y: point2.y, zIndex }), layout, onDrag: (event, gesturePoint) => {
...
function useCycle(...items) {
  ...
  const runCycle = (0, import_react48.useCallback)((next) => {
